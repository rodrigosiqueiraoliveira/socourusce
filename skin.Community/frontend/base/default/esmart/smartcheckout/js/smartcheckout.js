...
/* Login popup class */
var SmartCheckoutLoginPopup = Class.create({
    initialize: function(options) {
        this.options = options;
        this.popup_container = $('smartcheckout-login-popup');
        this.popup_link = $('smartcheckout-login-link');
        this.popup = null;
        this.createPopup();
        this.mode = 'login';

        this.forgot_password_link = $('smartcheckout-forgot-password-link');
        this.forgot_password_container = $('smartcheckout-login-popup-contents-forgot');
        this.forgot_password_loading = $('smartcheckout-forgot-loading');
        this.forgot_password_error = $('smartcheckout-forgot-error');
        this.forgot_password_success = $('smartcheckout-forgot-success');
        this.forgot_password_button = $('smartcheckout-forgot-button');
        this.forgot_password_table = $('smartcheckout-forgot-table');

        this.login_link = $('smartcheckout-return-login-link');
        this.login_container = $('smartcheckout-login-popup-contents-login');
        this.login_table = $('smartcheckout-login-table');
        this.login_error = $('smartcheckout-login-error');
        this.login_loading = $('smartcheckout-login-loading');
        this.login_button = $('smartcheckout-login-button');
        this.login_form = $('smartcheckout-login-form');
        this.login_username = $('id_smartcheckout_username');
        ...
        this.forgot_password_handler = function(e) {
            var email = $('id_smartcheckout_email').getValue();
...
function get_totals_element()
{
    ...
    var search_osc = $$('div.smartcheckout-summary');
            ...
            if(transport.status == 200)    {
                ...
                if(update_payments)    {
                    ...
                    $$('div.payment-methods input[name^=payment\[method\]]').invoke('observe', 'click', function() {
                        $$('div.smartcheckout-payment-method-error').each(function(item) {
...
function get_save_billing_function(url, set_methods_url, update_payments, triggered)
{
    ...
    return function()    {
        ...
        var shipment_methods = $$('div.smartcheckout-shipping-method-block')[0];
            ...
            if(transport.status == 200)    {
                if(shipment_methods_found)  {
                    ...
                    $$('dl.shipment-methods input').invoke('observe', 'click', function() {
                        $$('div.smartcheckout-shipment-method-error').each(function(item) {
                            new Effect.Fade(item);
                        });
                    });
                }
        ...
        new Ajax.Request(url, {
            ...
            if(transport.status == 200)    {
                ...
                if(update_payments)    {
                    ...
                    if($RF($('smartcheckout-form'), 'payment[method]') != null)    {
                        try    {
                            var payment_method = $RF(form, 'payment[method]');
                            $('container_payment_method_' + payment_method).show();
                            $('payment_form_' + payment_method).show();
...
function addressPreview(templates, target) {
    ...
    if(targetelem){
        targetelem.update('');
        targetelem.insert($$('table.smartcheckout-summary')[0].cloneNode(true));
        targetelem.insert($$('table.smartcheckout-totals')[0].cloneNode(true));
    }
}
