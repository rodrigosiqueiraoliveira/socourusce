 ...
 */
class Esmart_SmartCheckout_Block_Checkout extends Mage_Checkout_Block_Onepage_Abstract
{
    ...
    const SESSION_ADDRESS_CHECK_NAME = 'smartcheckout_address_check_name';
    ...
    {
        $this->settings = Mage::helper('smartcheckout/checkout')->loadConfig();
    }
    ...
    public function _handlePostData()
    {
        ...
        /**
         * @todo This false parameter never lets the test combination return true... Why?
         * 
         */
        if($this->_isLoggedInWithAddresses() && false) {
            ...
            if(!$this->_isLoggedIn()){
                ...
                if($registration_mode == 'auto_generate_account')   {
                    ...
                    $password = Mage::helper('smartcheckout/checkout')->generatePassword();
            ...
            } elseif(!empty($billing_data['vat_id'])){
            ...
            if($this->differentShippingAvailable()) {
                if(!isset($billing_data['use_for_shipping']) || $billing_data['use_for_shipping'] != '1')   {
                    ...
                    $shipping_result = Mage::helper('smartcheckout/checkout')->saveShipping($shipping_data, $shippingAddressId);
                    ...
                    $shipping_result = Mage::helper('smartcheckout/checkout')->saveShipping($billing_data, $customerAddressId);
                }
            }
        }
        ...
        if(!$this->isVirtual() )  {
            $result = Mage::helper('smartcheckout/checkout')->saveShippingMethod($shipping_method);
            if(isset($result['error']))    {
                $this->formErrors['shipping_method'] = true;
            } else {
        ...
        try {
            ...
            $result = Mage::helper('smartcheckout/checkout')->savePayment($payment);
