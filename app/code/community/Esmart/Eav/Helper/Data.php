...
class Esmart_Eav_Helper_Data extends Mage_Core_Helper_Abstract
{
    /**
    ...
    {
        Mage::throwException(Mage::helper('esmart_eav')->__('Use helper with defined EAV entity'));
    }
    ...
    public function getAttributeFormOptions()
    {
        return array(
            array(
                'label' => Mage::helper('esmart_eav')->__('Default EAV Form'),
                'value' => 'default'
            )
        );
    }
            ...
            case 'multiline':
                if (isset($validateRules['min_text_length']) && isset($validateRules['max_text_length'])) {
                    ...
                    if ($minTextLength > $maxTextLength) {
                        $errors[] = Mage::helper('esmart_eav')->__('Wrong values for minimum and maximum text length validation rules.');
                    }
                }
                break;
            case 'date':
                if (isset($validateRules['date_range_min']) && isset($validateRules['date_range_max'])) {
                    ...
                    if ($minValue > $maxValue) {
                        $errors[] = Mage::helper('esmart_eav')->__('Wrong values for minimum and maximum date validation rules.');
                    }
                }
                break;
            default:
    ...
    public function getAttributeInputTypes($inputType = null)
    {
        $inputTypes = array(
            'text'          => array(
                'label'             => Mage::helper('esmart_eav')->__('Text Field'),
            ...
            'textarea'      => array(
                'label'             => Mage::helper('esmart_eav')->__('Text Area'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'min_text_length',
                    'max_text_length',
            ...
            'multiline'     => array(
                'label'             => Mage::helper('esmart_eav')->__('Multiple Line'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'min_text_length',
                    'max_text_length',
            ...
            'date'          => array(
                'label'             => Mage::helper('esmart_eav')->__('Date'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'date_range_min',
                    'date_range_max'
            ...
            'select'        => array(
                'label'             => Mage::helper('esmart_eav')->__('Dropdown'),
                'manage_options'    => true,
            ...
            'multiselect'   => array(
                'label'             => Mage::helper('esmart_eav')->__('Multiple Select'),
                'manage_options'    => true,
            ...
            'boolean'       => array(
                'label'             => Mage::helper('esmart_eav')->__('Yes/No'),
                'manage_options'    => false,
                'validate_types'    => array(),
                'validate_filters'  => array(),
                'filter_types'      => array(),
            ...
            'file'          => array(
                'label'             => Mage::helper('esmart_eav')->__('File (attachment)'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'max_file_size',
                    'file_extensions'
            ...
            'image'         => array(
                'label'             => Mage::helper('esmart_eav')->__('Image File'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'max_file_size',
                    'max_image_width',
    ...
    {
        return array(
            'alphanumeric'  => Mage::helper('esmart_eav')->__('Alphanumeric'),
            'numeric'       => Mage::helper('esmart_eav')->__('Numeric Only'),
            'alpha'         => Mage::helper('esmart_eav')->__('Alpha Only'),
            'url'           => Mage::helper('esmart_eav')->__('URL'),
            'email'         => Mage::helper('esmart_eav')->__('Email'),
            'date'          => Mage::helper('esmart_eav')->__('Date'),
        );
    }
    ...
    {
        return array(
            'striptags'     => Mage::helper('esmart_eav')->__('Strip HTML Tags'),
            'escapehtml'    => Mage::helper('esmart_eav')->__('Escape HTML Entities'),
            'date'          => Mage::helper('esmart_eav')->__('Normalize Date')
        );
    }
        ...
        if ($data) {
            ...
            if (isset($data['attribute_code'])) {
                ...
                if (!$validatorAttrCode->isValid($data['attribute_code'])) {
                    Mage::throwException(Mage::helper('esmart_eav')->__('Attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscore(_) in this field, first character should be a letter.'));
                }
            }
        }
        return $data;
