 ...
 */
class Esmart_Logging_Model_Observer
{

    /**
     * Instance of Esmart_Logging_Model_Logging
     *
     * @var object Esmart_Logging_Model_Logging
     */
    protected $_processor;

    /**
     * Initialize Esmart_Logging_Model_Processor class
     *
     */
    public function __construct()
    {
        $this->_processor = Mage::getSingleton('esmart_logging/processor');
    }
    ...
    public function adminSessionLoginFailed($observer)
    {
        ...
        if (class_exists('Esmart_Pci_Model_Observer', false) && $eventModel) {
            $exception = $observer->getException();
            if ($exception->getCode() == Esmart_Pci_Model_Observer::ADMIN_USER_LOCKED) {
                $eventModel->setInfo(Mage::helper('esmart_logging')->__('User is locked'))->save();
            }
        }
    }
    ...
    protected function _logAdminLogin($username, $userId = null)
    {
        ...
        if (!Mage::getSingleton('esmart_logging/config')->isActive($eventCode, true)) {
            return;
        }
        $success = (bool)$userId;
        if (!$userId) {
        ...
        return Mage::getSingleton('esmart_logging/event')->setData(array(
    ...
    {
        $lastRotationFlag = Mage::getModel('esmart_logging/flag')->loadSelf();
        $lastRotationTime = $lastRotationFlag->getFlagData();
        $rotationFrequency = 3600 * 24 * (int)Mage::getConfig()->getNode('default/esmart_logging/rotation/frequency');
        if (!$lastRotationTime || ($lastRotationTime < time() - $rotationFrequency)) {
            Mage::getResourceModel('esmart_logging/event')->rotate(
                3600 * 24 *(int)Mage::getConfig()->getNode('default/esmart_logging/rotation/lifetime')
            );
        }
