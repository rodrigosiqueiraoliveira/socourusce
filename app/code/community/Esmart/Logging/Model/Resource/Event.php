...
class Esmart_Logging_Model_Resource_Event extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
     * Initialize resource
     *
    ...
    {
        $this->_init('esmart_logging/event', 'log_id');
    }
...
//        try {
            ...
            $table = $this->getTable('esmart_logging/event');
            ...
            if ($latestLogEntry) {
                ...
                $archive = Mage::getModel('esmart_logging/archive');
                $archive->createNew();

                $expr = Mage::getResourceHelper('esmart_logging')->getInetNtoaExpr('ip');
                $select = $readAdapter->select()
...
//        }
    ...
    public function getUserNames()
    {
        ...
        $select = $adapter->select()
            ...
            ->joinInner(
                array('events' => $this->getTable('esmart_logging/event')),
    ...
    public function getEventChangeIds($eventId)
    {
        $adapter = $this->_getReadAdapter();
        $select = $adapter->select()
            ->from($this->getTable('esmart_logging/event_changes'), array('id'))
