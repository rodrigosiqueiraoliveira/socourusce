 ...
 *
 */
class Esmart_Logging_Model_Handler_Controllers
{
     ...
     * @param Esmart_Logging_Model_Event $eventModel
    ...
    public function postDispatchGeneric($config, $eventModel, $processorModel)
    {
        if ($collectedIds = $processorModel->getCollectedIds()) {
            $eventModel->setInfo(Mage::helper('esmart_logging')->implodeValues($collectedIds));
            return true;
        }
        return false;
    }
    ...
    */
     ...
     * @param Varien_Simplexml_Element $config
     * @param Esmart_Logging_Model_Event $eventModel
     * @param Esmart_Logging_Model_Processor $processor
     * @return Esmart_Logging_Model_Event
     */
    public function postDispatchConfigSave($config, $eventModel, $processor)
    {
        ...
        $change = Mage::getModel('esmart_logging/event_changes');
     ...
     * @param Varien_Simplexml_Element $config
     * @param Esmart_Logging_Model_Event $eventModel
     * @return Esmart_Logging_Model_Event|false
     */
    public function postDispatchForgotPassword($config, $eventModel)
    {
        if (Mage::app()->getRequest()->isPost()) {
            if ($model = Mage::registry('esmart_logging_saved_model_adminhtml_index_forgotpassword')) {
                $info = $model->getId();
            } else {
     ...
     * @param Varien_Simplexml_Element $config
     * @param Esmart_Logging_Model_Event $eventModel
     * @param Esmart_Logging_Model_Processor $processorModel
     * @return Esmart_Logging_Model_Event
     */
    public function postDispatchPromoCatalogSaveAndApply($config, $eventModel, $processorModel)
    {
        ...
        if ($request->getParam('auto_apply')) {
            $eventModel->setInfo(Mage::helper('esmart_logging')->__('%s & applied', $eventModel->getInfo()));
        }
    ...
    public function postDispatchTaxRatesImport($config, $eventModel)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('esmart_logging')->__('Tax Rates Import'));
    }
    ...
    public function postDispatchProductUpdateAttributes($config, $eventModel, $processor)
    {
        ...
        return $eventModel->setInfo(Mage::helper('esmart_logging')->__('Attributes Updated'));
    }
    ...
    public function postDispatchCustomerSegmentMatch($config, $eventModel)
    {
        /*
        $request = Mage::app()->getRequest();
        $customersQty = Mage::getModel('esmart_customersegment/segment')->getResource()
                ->getSegmentCustomersQty($request->getParam('id'));
        return $eventModel->setInfo(
            $request->getParam('id') ?
                Mage::helper('esmart_customersegment')->__('Matched %d Customers of Segment %s', $customersQty, $request->getParam('id')) :
                '-'
        );
        */
    ...
    public function postDispatchSystemBackupsDelete($config, $eventModel)
    {
        ...
        if ($backup) {
            $eventModel->setIsSuccess($backup->getIsSuccess())
                ->setInfo(Mage::helper('esmart_logging')->implodeValues($backup->getDeleteResult()));
        } else {
            $eventModel->setIsSuccess(false);
        }
        return $eventModel;
    ...
    public function postDispatchSystemCurrencySave($config, $eventModel, $processor)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('esmart_logging')->__('Currency Rates Saved'));
    }
    ...
    public function postDispatchSaveCacheSettings($config, $eventModel, $processor)
    {
        ...
        if (is_array($cacheTypes) && !empty($cacheTypes)) {
            ...
            $info = Mage::helper('esmart_logging')->__('Cache types: %s ', $cacheTypes);
        }
    ...
    public function postDispatchTaxRatesExport($config, $eventModel)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('esmart_logging')->__('Tax Rates Export'));
    }
