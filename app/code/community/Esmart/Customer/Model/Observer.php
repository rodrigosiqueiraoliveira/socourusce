<?php
class Esmart_Customer_Model_Observer
{
    ...
    public function salesQuoteAddressCollectionAfterLoad(Varien_Event_Observer $observer)
    {
        ...
        if ($collection instanceof Varien_Data_Collection_Db) {
            Mage::getModel('esmart_customer/sales_quote_address')
    ...
    public function salesOrderAfterLoad(Varien_Event_Observer $observer)
    {
        ...
        if ($order instanceof Mage_Core_Model_Abstract) {
            Mage::getModel('esmart_customer/sales_order')
                ->load($order->getId())
    ...
    public function salesOrderAddressCollectionAfterLoad(Varien_Event_Observer $observer)
    {
        ...
        if ($collection instanceof Varien_Data_Collection_Db) {
            Mage::getModel('esmart_customer/sales_order_address')
                ->attachDataToCollection($collection);
        }
     ...
     * @return Esmart_Customer_Model_Observer
     */
    public function esmartCustomerAttributeBeforeSave(Varien_Event_Observer $observer)
    {
     ...
     * @return Esmart_Customer_Model_Observer
     */
    public function esmartCustomerAttributeSave(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute && $attribute->isObjectNew()) {
            Mage::getModel('esmart_customer/sales_quote')
     ...
     * @return Esmart_Customer_Model_Observer
     */
    public function esmartCustomerAttributeDelete(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute && !$attribute->isObjectNew()) {
            Mage::getModel('esmart_customer/sales_quote')
     ...
     * @return Esmart_Customer_Model_Observer
     */
    public function esmartCustomerAddressAttributeSave(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute && $attribute->isObjectNew()) {
            Mage::getModel('esmart_customer/sales_quote_address')
     ...
     * @return Esmart_Customer_Model_Observer
     */
    public function esmartCustomerAddressAttributeDelete(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute && !$attribute->isObjectNew()) {
            Mage::getModel('esmart_customer/sales_quote_address')
    ...
    protected function _copyFieldset(Varien_Event_Observer $observer, $algoritm, $convertType)
    {
        ...
        if ($source instanceof Mage_Core_Model_Abstract && $target instanceof Mage_Core_Model_Abstract) {
            if ($convertType == self::CONVERT_TYPE_CUSTOMER) {
                $attributes = Mage::helper('esmart_customer')->getCustomerUserDefinedAttributeCodes();
                $prefix     = 'customer_';
            } else if ($convertType == self::CONVERT_TYPE_CUSTOMER_ADDRESS) {
                $attributes = Mage::helper('esmart_customer')->getCustomerAddressUserDefinedAttributeCodes();
                $prefix     = '';
            } else {
                return $this;
            }
