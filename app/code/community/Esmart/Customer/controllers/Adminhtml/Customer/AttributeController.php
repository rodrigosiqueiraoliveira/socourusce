 ...
 */
class Esmart_Customer_Adminhtml_Customer_AttributeController extends Mage_Adminhtml_Controller_Action
{
    /**
     ...
     *
     * @return Esmart_Customer_Adminhtml_Customer_AttributeController
     */
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                ...
                Mage::helper('esmart_customer')->__('Customer'))
            ->_addBreadcrumb(
                Mage::helper('esmart_customer')->__('Manage Customer Attributes'),
                Mage::helper('esmart_customer')->__('Manage Customer Attributes'));
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()
                    ->addError(Mage::helper('esmart_customer')->__('Attribute is no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('esmart_customer')->__('Edit Customer Attribute')
            : Mage::helper('esmart_customer')->__('New Customer Attribute');
    ...
    {
        return Mage::helper('esmart_customer/customer')->filterPostData($data);
    }
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $data) {
            ...
            try {
                Mage::dispatchEvent('esmart_customer_attribute_before_save', array(
                    'attribute' => $attributeObject
                ));
                $attributeObject->save();
                Mage::dispatchEvent('esmart_customer_attribute_save', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('esmart_customer')->__('The customer attribute has been saved.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('esmart_customer')->__('An error occurred while saving the customer attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            try {
                ...
                Mage::dispatchEvent('esmart_customer_attribute_delete', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('esmart_customer')->__('The customer attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Mage_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('esmart_customer')->__('An error occurred while deleting the customer attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
