<?php
class RedFeet_Navigation_Model_Layer_Filter_Category extends GoMage_Navigation_Model_Layer_Filter_Category
{
    protected function _getItemsData()
    {
        ...
        if ($data === null) {
            $category   = $this->getLayer()->getRootCategory();
            /** @var $categoty Mage_Catalog_Model_Categeory */
            //$categories = $categoty->getChildrenCategories();
            
            if (Mage::getStoreConfigFlag('gomage_navigation/category/show_allsubcats')){
                $cats_ids = array_diff($category->getAllChildren(true, array(75)), array($category->getId()));
                if (count($cats_ids)) {
