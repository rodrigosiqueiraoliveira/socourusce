...
class RedFeet_Navigation_Block_Navigation extends Mage_Core_Block_Template
{
  ...
  public function getIsShowAllSubcategories(){
     ...
     * @return string
     */
    public function getCategoryUrl($category, $asGomageAjaxUrl = false)
    {
        if ($category instanceof Mage_Catalog_Model_Category) {
            $url = $category->getUrl($asGomageAjaxUrl);
        } else {
            $url = $this->_getCategoryInstance()
                ->setData($category->getData())
                ->getUrl($asGomageAjaxUrl);
        }
        return $url;
    }
    ...
    protected function _renderCategoryMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false,
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false, $asGomageAjaxUrl = false)
    {
        ...
        switch ($this->getTypeNavigation())
        {
            case GoMage_Navigation_Model_Layer::FILTER_TYPE_DROPDOWN:
                ...
                if ($category->getLevel() == $this->_root_level)
                {
                    ...
                    $option_value = ($this->getIsAjax() ? $this->getAjaxUrl($category) : $this->getCategoryUrl($category, $asGomageAjaxUrl) );
            ...
            case GoMage_Navigation_Model_Layer::FILTER_TYPE_PLAIN: 
             ...
             if ($category->getData('navigation_pw_ob_show')){
                ...
                $this->_offer_block_html .= '</div>';
                    ...
                    $htmlA = '<a href="'.$this->getCategoryUrl($category, $asGomageAjaxUrl).'"'.$linkClass;
                    if ($this->getIsAjax())
                    {
                ...
                else
                {
                     ...
                     if ($category->getLevel() == ($this->_root_level + 1))
                     {
                          if ($this->_navigation_place == self::MENU_BAR)
                              ...
                              Mage::unregister('menu_count');
                            Mage::register('menu_count', 0);
                     }
                     Mage::unregister('menu_children');
                     Mage::register('menu_children', (($this->_childs_count > 0) && ($category->getLevel() == ($this->_root_level + 1))));
                     
                     if (($this->_childs_count == 1) || ($_cat_column && ($_cat_column != $this->_current_column)) || (Mage::registry('menu_count') != 0 && Mage::registry('menu_count') % 10 == 0))
                     {
                        $this->_current_column = $_cat_column;                          

                        if ($this->_childs_count != 1 || Mage::registry('menu_count') % 10 != 0)
                           $html[] = '</ul>';
                  ...
                  $_ul_new_class = '';
                  if(Mage::registry('menu_count') != 0 && Mage::registry('menu_count') % 10 == 0) {
                      $_ul_new_class .= ' gan-plain-item-extra-column';
                  }
                  else if($hasActiveChildren && $category->getLevel() == ($this->_root_level + 1)){
                      $_ul_new_class .= ' gan-plain-item-border-column';
                  }
      
                        $html[] = '<ul style="'. $_ul_styles . '" class="gan-plain-item'.$_ul_new_class.'">';
                     }
                    ...
                    $newValue = (Mage::registry('menu_count') + 1);
                    Mage::unregister('menu_count');
                    Mage::register('menu_count', $newValue);
                      }
                     }
                   ...
                   $category_view = 0;
                     ...
                     $htmlA = '<a style="padding-left:' .  (10*($category->getLevel() - ($this->_root_level + 1))) . 'px;" href="'.$this->getCategoryUrl($category, $asGomageAjaxUrl).'"'.$linkClass;                              
                     if ($this->getIsAjax())
                     {
                        $htmlA .= ' onclick="setNavigationUrl(\'' . $this->getAjaxUrl($category) . '\'); return false;" ';
                     }
      ...
      $asGomageAjaxUrl
                ...
                {  
                    ...
                    if ($hasActiveChildren)
                    {
                     ...
                     /* Adding div that contains arrow */
                     $html[] = '<div class="socourus-menu-arrow">&nbsp;</div>';
                    }   
                    $html[] = '</li>';
                }    
            ...
            case GoMage_Navigation_Model_Layer::FILTER_TYPE_FOLDING:
                ...
                $htmlA  = '<a href="'.$this->getCategoryUrl($category, $asGomageAjaxUrl).'"';
            ...
            default:                                   
                ...
                {                                
                    ...
                    $html[] = '<a href="'.$this->getCategoryUrl($category, $asGomageAjaxUrl).'"'.$linkClass.'>';                    
     ...
     * @return string
     */
    public function renderCategoriesMenuHtml($level = 0, $outermostItemClass = '', $childrenWrapClass = '', $categoryIdToStart = null, $asGomageAjaxUrl = false)
    {
        $activeCategories = array();

        $_root_category = ($categoryIdToStart != null) ? $categoryIdToStart : Mage::app()->getStore()->getRootCategoryId();
        switch ($this->_navigation_place)
        {            
            case self::LEFT_COLUMN :  
            case self::RIGTH_COLUMN :
    ...
    public function getCategoriesMenuAsArray($level = 0, $outermostItemClass = '', $childrenWrapClass = '', $categoryIdToStart = null, $asGomageAjaxUrl = false)
    {
        $activeCategories = array();
  ...
  $arrayTree = array();
    ...
    public function setCurrentCategoryKey($key = null)
    {
  if(is_null($key)) $key = $this->getCurrenCategoryKey();
        $this->_currentCategoryKey = $key;
    }
}